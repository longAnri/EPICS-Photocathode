...
// ----------------------------------------------------------------
// CRC14 special (exclude sync but include length + message)
// ----------------------------------------------------------------
static uint32_t crc14(const uint8_t* data, size_t len, uint32_t init)
{
    uint32_t crc = init & 0x3FFF;
...
//    name      func              init        xorout  bytes  chk("123456789")
	...
	{"crc14",	crc14,			  0x3FFF,	  0x0000,	  2}  // crc14 custom
};
...
printPseudo(const StreamFormat& format, StreamBuffer& output)
{
	...
	// CRC14 special
	if (strcmp(checksumMap[fnum].name, "crc14") == 0)
	{
    	// raw CRC14 = 14bit
    	uint32_t rawcrc = sum & 0x3FFF;

    	// 按 big endian 输出 2 字节 HEX ASCII
    	uint8_t high = ((rawcrc >> 7) & 0x7F) + 34;
    	uint8_t low  = (rawcrc & 0x7F) + 34;

    	// 使用 format.flags 判断是否要 ASCII
    	// 这里我们按照 HEX ASCII 输出两个字节
    	// output.print("<%02" PRIX8 "><%02" PRIX8 ">", low, high);
		// output.print("%02" PRIX8 "%02" PRIX8, low, high);
		output.append((char*)&low,  1);
    	output.append((char*)&high, 1);
		return true;
	}
...
scanPseudo(const StreamFormat& format, StreamBuffer& input, size_t& cursor)
{
	...
	if (strcmp(checksumMap[fnum].name, "crc14") == 0)
	{
    	...
    	// 期望的两个 hex ASCII 字节
    	uint8_t expected_hi = ((rawcrc >> 7) & 0x7F) + 34;
    	uint8_t expected_lo = (rawcrc & 0x7F) + 34;

    	// 确认至少两个 HEX 字节存在
    	if (input.length() - cursor < 2)  // 因为是 HEX 输出，每个字节两字符
        	return -1;

    //	unsigned int parsed_hi, parsed_lo;
    	// 扫描十六进制字符
    	//if (sscanf(input.expand(cursor,1)(), "<") != 0)
        //	return -1;
    	//if (sscanf(input.expand(cursor+1,2)(), "%2x", &parsed_hi) != 1)
        //	return -1;
    	//if (sscanf(input.expand(cursor+3,1)(), ">") != 0)
        //	return -1;


    	//if (sscanf(input.expand(cursor+4,1)(), "<") != 0)
        //	return -1;
    	//if (sscanf(input.expand(cursor+5,2)(), "%2x", &parsed_lo) != 1)
        //	return -1;
    	//if (sscanf(input.expand(cursor+7,1)(), ">") != 0)
        //	return -1;
	//	if (sscanf(input.expand(cursor,2)(), "%2x", &parsed_hi) != 1)
      //      return -1;
       // if (sscanf(input.expand(cursor+2,2)(), "%2x", &parsed_lo) != 1)
      //      return -1;

    //	if ((uint8_t)parsed_hi == expected_hi &&
    //    	(uint8_t)parsed_lo == expected_lo)
    //	{
    //    	return 4;  // 成功，consumed 4 个字符
    //	}
		uint8_t recv_low  = input[cursor];
    	uint8_t recv_high = input[cursor + 1];

    	if (recv_low == expected_lo &&
        	recv_high == expected_hi)
    	{
        	return 2;  // 消耗 2 个字节
    	}
    	return -1;
